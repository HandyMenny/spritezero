 ...
 * @param   {Object[]}              [options.imgs]                 Array of `{ buf: Buffer, id: String, format: 'svg'|'png' }`
...
/**
 * Same as generateLayout but can be used to dedupe identical icons
 * and still preserve the reference.
 *
 * For example if `A.buf` and `B.buf` are identical, a single icon
 ...
 * @param   {number}                [options.pixelRatio]           Ratio of a 72dpi screen pixel to the destination pixel density
 * @param   {boolean}               [options.format]               If true, generate {@link DataLayout} as first argument to callback and {@link ImgLayout} as the second argument; if false, generate only {@link ImgLayout} as the first argument to the callback function
 * @param   {boolean}               [options.unique]               If true, deduplicate identical images
 * @param   {boolean}               [options.maxIconSize]          optional, overrides the max_size in mapnik
 * @param   {boolean}               [options.removeOversizedIcons] optional, if set, filters out icons that mapnik says are too big
...
function generateLayoutInternal(options, callback) {
    ...
    if (options.unique) {
        /* If 2 items are pointing to identical buffers (icons)
        ...
        */
        ...
        /* The signature of each item */
        var perItemId = {};

        /* The items for each signature */
        var itemIdsPer = {};

        options.imgs.forEach((item) => {
            var buf = item.buf.toString('base64');

            perItemId[item.id] = buf;

            if (buf in itemIdsPer) {
                itemIdsPer[buf].push(item.id);
            } else {
                itemIdsPer[buf] = [item.id];
            }
        });

        /* Only keep 1 item per signature for packing */
        options.imgs = options.imgs.filter((item) => {
            var buf = perItemId[item.id];
            return item.id === itemIdsPer[buf][0];
        });
    }

    function createImages(img, callback) {
        var format = img.format ? img.format.toLowerCase() : 'svg';
        if (format !== 'svg' && format !== 'png') callback(new Error('only svg and png images supported'));
        var isSvg = format === 'svg';
        var mapnikOpts = isSvg ? { scale: options.pixelRatio } : {};
        if (options.maxIconSize) {
            mapnikOpts.max_size = options.maxIconSize;
        }
        var imageFn = isSvg ? mapnik.Image.fromSVGBytes : mapnik.Image.fromBytes;
        imageFn(img.buf, mapnikOpts, (err, image) => {
            if (err && err.message.match(/image created from (svg|bytes) must be \d+ pixels or fewer on each side/) && options.removeOversizedIcons) return callback(null, null);
            ...
            if (
                ...
                options.format &&
                (img.buf.includes('mapbox-stretch') || img.buf.includes('mapbox-text-placeholder'))
            ) {
                const metaOps = { svg: img.buf, pixelRatio: options.pixelRatio };
                ...
                return callback(null, {
                  ...
                  width: image.width(),
                  height: image.height(),
                  pixelRatio: isSvg ? options.pixelRatio : 1,
                  buffer: image
                });
            }
        });
    ...
    q.awaitAll((err, imagesWithSizes) => {
        ...
        if (options.format) {
            ...
            imagesWithSizes.forEach((item) => {
                ...
                if (options.unique) {
                    var buf = perItemId[item.id];
                    itemIdsToUpdate = itemIdsPer[buf];
                }
